#!/usr/bin/env python3
"""Run a CARLA OpenDRIVE closed loop, replay instances, and render live 3DGS.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--instances-info",
        type=Path,
        help="Optional DriveStudio instances/instances_info.json to replay as CARLA actors.",
    )
    parser.add_argument(
    ...
    parser.add_argument("--instance-class-prefix", default="vehicle.", help="Only replay instance classes with this prefix.")
    ...
    parser.add_argument(
        ...
        help=(
            "raw-local treats instances_info obj_to_world as already in sequence-local world. "
    ...
    parser.add_argument(
        "--control-mode",
        choices=("path", "steer-bias", "constant-steer"),
        default="path",
        help=(
            "path follows the reference route; steer-bias adds --steer-bias to the path steering command; "
            "constant-steer keeps speed control but replaces steering with --steer-bias."
        ),
    )
    parser.add_argument(
        "--steer-bias",
        type=float,
        default=0.0,
        help="Steering offset/value used by steer-bias and constant-steer. Positive values steer right in CARLA.",
    )
    ...
    parser.add_argument("--collision-output", type=Path, help="Optional CSV for CARLA collision sensor events.")
...
def apply_control_mode(carla, args: argparse.Namespace, control, steer_limit: float) -> tuple:
    base_steer = float(control.steer)
    steer_bias = float(args.steer_bias) if args.control_mode in ("steer-bias", "constant-steer") else 0.0
    if args.control_mode == "constant-steer":
        final_steer = clamp(steer_bias, -steer_limit, steer_limit)
    else:
        final_steer = clamp(base_steer + steer_bias, -steer_limit, steer_limit)
    ...
    return control, base_steer, steer_bias
...
def attach_collision_sensor(carla, world, hero):
    blueprint_library = world.get_blueprint_library()
    blueprint = blueprint_library.find("sensor.other.collision")
    sensor = world.spawn_actor(blueprint, carla.Transform(), attach_to=hero)
    events: list[dict] = []
    ...
    def on_collision(event) -> None:
        ...
        events.append(
            {
                "sim_frame": int(event.frame),
                "other_actor_id": int(other.id) if other is not None else -1,
                "other_actor_type": getattr(other, "type_id", "") if other is not None else "",
...
def main() -> int:
    ...
    if args.control_mode == "path" and abs(args.steer_bias) > 1e-9:
        print("[control] --steer-bias is ignored when --control-mode path")
    if args.control_mode != "path" and args.render_pose_source == "reference":
        print(
            "[control] warning: --render-pose-source reference renders the original route; "
            "use --render-pose-source actor to view the biased hero trajectory."
        )
    print(f"[control] mode={args.control_mode} steer_bias={args.steer_bias:.4f} max_steer={args.max_steer:.4f}")
    ...
    for _ in range(max(0, args.follow_settle_ticks)):
        ...
        world.tick()
    collision_sensor, collision_events = attach_collision_sensor(carla, world, hero)
    ...
    try:
        for step in range(max_steps):
            ...
            control, base_steer, steer_bias = apply_control_mode(carla, args, control, args.max_steer)
            ...
            if should_render:
                ...
                print(
                    ...
                    f"track_error_m={nearest_dist:.3f} steer={float(control.steer):.3f} "
                    f"collisions={len(collision_events)} avg_render_fps={rendered / max(elapsed, 1e-9):.2f}"
                )
    ...
    profiler.print()
    print(
        f"[live-3dgs] controller_steps={len(pose_rows)} rendered_frames={rendered} "
        f"instance_actors={len(instance_actors)} instance_rows={len(instance_rows)}"
    )
    return 0
